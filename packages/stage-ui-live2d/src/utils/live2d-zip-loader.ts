...
import { resolveArchivePath } from './live2d-validator'
...
ZipLoader.createSettings = async (reader: JSZip) => {
  ...
  const settings = await (async () => {
    ...
    return createModelSettings(registerUnreferencedResources(settingsText, settingsFilePath, filePaths), settingsFilePath)
  })()
...
function createArchiveReference(settingsPath: string, assetPath: string): string {
  const settingsDirectory = settingsPath.split(/[\\/]/).slice(0, -1)
  const assetSegments = assetPath.split(/[\\/]/)
  let sharedSegmentCount = 0

  while (
    sharedSegmentCount < settingsDirectory.length
    && sharedSegmentCount < assetSegments.length
    && settingsDirectory[sharedSegmentCount] === assetSegments[sharedSegmentCount]
  ) {
    sharedSegmentCount += 1
  }

  return [
    ...settingsDirectory.slice(sharedSegmentCount).map(() => '..'),
    ...assetSegments.slice(sharedSegmentCount),
  ].join('/')
}

function resourceName(resourcePath: string, suffix: string): string {
  return basename(resourcePath).slice(0, -suffix.length)
}

/**
 * Adds loose Cubism motion and expression files to runtime-only model settings.
 *
 * Some exported model archives contain valid resources beside model3.json but omit
 * them from FileReferences. The archive remains unchanged; only its loaded settings
 * gain references so the renderer can discover and play those resources.
 */
function registerUnreferencedResources(text: string, settingsPath: string, archivePaths: string[]): string {
  const json = JSON.parse(text) as Record<string, unknown>
  if (!isRecord(json.FileReferences))
    return text

  const fileReferences = json.FileReferences
  const declaredMotionGroups = isRecord(fileReferences.Motions)
    ? fileReferences.Motions
    : {}
  const referencedMotions = new Set<string>()

  for (const motionEntries of Object.values(declaredMotionGroups)) {
    if (!Array.isArray(motionEntries))
      continue

    for (const entry of motionEntries) {
      if (isRecord(entry) && typeof entry.File === 'string')
        referencedMotions.add(resolveArchivePath(settingsPath, entry.File))
    }
  }

  for (const motionPath of archivePaths.filter(path => path.toLowerCase().endsWith('.motion3.json'))) {
    if (referencedMotions.has(motionPath))
      continue

    const motionName = resourceName(motionPath, '.motion3.json')
    const currentEntries = declaredMotionGroups[motionName]
    const motionEntries = Array.isArray(currentEntries) ? currentEntries : []
    motionEntries.push({ File: createArchiveReference(settingsPath, motionPath) })
    declaredMotionGroups[motionName] = motionEntries
  }

  if (Object.keys(declaredMotionGroups).length > 0)
    fileReferences.Motions = declaredMotionGroups

  const declaredExpressions = Array.isArray(fileReferences.Expressions)
    ? fileReferences.Expressions.filter(isRecord)
    : []
  const referencedExpressions = new Set(
    declaredExpressions
      .map(entry => typeof entry.File === 'string' ? resolveArchivePath(settingsPath, entry.File) : '')
      .filter(Boolean),
  )

  for (const expressionPath of archivePaths.filter(path => path.toLowerCase().endsWith('.exp3.json'))) {
    if (referencedExpressions.has(expressionPath))
      continue

    declaredExpressions.push({
      Name: resourceName(expressionPath, '.exp3.json'),
      File: createArchiveReference(settingsPath, expressionPath),
    })
  }

  if (declaredExpressions.length > 0)
    fileReferences.Expressions = declaredExpressions
...
FileLoader.createSettings = async (files: File[]) => {
  ...
  const filePaths = files
    .filter(file => !shouldIgnoreLive2DArchiveEntry(file.webkitRelativePath || file.name))
    .map(file => file.webkitRelativePath || file.name)
  const settings = createModelSettings(registerUnreferencedResources(settingsText, settingsUrl, filePaths), settingsUrl)
