...
describe('opfs cache full directory persistence', () => {
  ...
  it('uses the selected model file when its temporary blob URL has expired', async () => {
    const sourceBlob = new File([new Uint8Array([1, 2, 3, 4])], 'huohuo.zip', {
      type: 'application/zip',
    })
    ...
    // ROOT CAUSE:
    //
    // Model selection can replace and revoke a temporary object URL while two
    // Live2D views initialize. Fetching that stale URL makes the selected model
    // disappear even though its IndexedDB File is still valid.
    //
    // We pass the durable File with the URL and read it directly on cache misses.
