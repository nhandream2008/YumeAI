...
describe('live2d zip loader settings sanitization', () => {
  ...
  it('registers unreferenced motion and expression files for runtime playback', async () => {
    await import('./live2d-zip-loader')
    const { ZipLoader } = await import('pixi-live2d-display/cubism4')

    const zip = new JSZip()
    zip.file('huohuo/huohuo.model3.json', JSON.stringify({
      Version: 3,
      FileReferences: {
        Moc: 'huohuo.moc3',
        Textures: ['huohuo.8192/texture_00.png'],
      },
      Groups: [],
    }))
    zip.file('huohuo/huohuo.moc3', new Uint8Array([77, 79, 67, 51]))
    zip.file('huohuo/huohuo.8192/texture_00.png', new Uint8Array([1, 2, 3]))
    zip.file('huohuo/haoqi.motion3.json', JSON.stringify({ Version: 3, Curves: [] }))
    zip.file('huohuo/angry.exp3.json', JSON.stringify({ Type: 'Live2D Expression', Parameters: [] }))
    ...
    expect(runtimeSettings.motions).toEqual({
      haoqi: [{ File: 'haoqi.motion3.json' }],
    })
    expect(runtimeSettings.expressions).toEqual([
