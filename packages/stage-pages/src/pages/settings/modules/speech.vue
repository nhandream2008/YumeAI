...
const voiceSearchQuery = shallowRef('')
const useSSML = shallowRef(false)
const testText = shallowRef('Hello, my name is AI Assistant')
...
const providerSelectionAttempted = shallowRef(false)
...
const hasSelectedSpeechProvider = computed(() => Boolean(
...
async function selectSpeechSource(sourceId: string) {
  providerSelectionAttempted.value = true
  errorMessage.value = ''
...
async function loadSelectedSpeechSource(providerId: string) {
  if (!providerId || providerId === 'speech-noop')
    return

  if (providerId === OFFICIAL_SPEECH_PROVIDER_ID || providerId === OFFICIAL_SPEECH_STREAMING_PROVIDER_ID) {
    await Promise.all([
      providersStore.fetchModelsForProvider(OFFICIAL_SPEECH_PROVIDER_ID),
      providersStore.fetchModelsForProvider(OFFICIAL_SPEECH_STREAMING_PROVIDER_ID),
    ])
  }
  else if (providersStore.supportsModelListing(providerId)) {
    await providersStore.fetchModelsForProvider(providerId)
  }

  await speechStore.loadVoicesForProvider(providerId, activeSpeechModel.value || undefined)
}

onMounted(async () => {
  try {
    await loadSelectedSpeechSource(activeSpeechProvider.value)
...
watch(activeSpeechProvider, async (newProvider) => {
  try {
    await loadSelectedSpeechSource(newProvider)
...
<template>
  ...
  <div :class="['flex flex-col gap-8', 'w-full']">
    <section :class="['flex flex-col gap-6 rounded-2xl p-5', 'w-full glass-tier-1']">
      <div class="flex flex-col gap-4">
        ...
        <div class="max-w-full">
          <fieldset
            v-if="selectableSpeechSources.length > 0"
          ...
          >
            ...
            <RouterLink
              ...
              :class="[
                'relative min-h-24 min-w-0 w-full flex flex-col items-center justify-center rounded-xl border-2 p-4',
                'border-neutral-100 bg-white transition-all duration-200 ease-in-out hover:border-primary-500/30',
                'dark:border-neutral-900 dark:bg-neutral-900/20 dark:hover:border-primary-400/30',
              ]"
            >
            ...
            >
              ...
              <div class="flex flex-col">
                <span class="font-medium">
                  {{ t('settings.pages.modules.speech.sections.section.provider-voice-selection.not-configured.title') }}
                </span>
                <span class="text-sm text-neutral-400 dark:text-neutral-500">
                  {{ t('settings.pages.modules.speech.sections.section.provider-voice-selection.not-configured.description') }}
                </span>
              </div>
      ...
      <div v-if="hasSelectedSpeechProvider">
        <div flex="~ col gap-4">
          <div>
          ...
          <div v-if="activeSpeechProvider === 'openai-compatible-audio-speech'">
            <FieldInput
              ...
              :label="t('settings.pages.modules.speech.sections.section.model.label')"
              :description="t('settings.pages.modules.speech.sections.section.model.description')"
              placeholder="tts-1"
          ...
          <div v-else-if="supportsModelListing" class="flex flex-col gap-4">
            ...
            <template v-else-if="displayedModelError">
              ...
              <FieldInput
                :model-value="activeSpeechModel || ''"
                :label="t('settings.pages.modules.speech.sections.section.model.label')"
                :description="t('settings.pages.modules.speech.sections.section.model.discovery-failed')"
            ...
            <!-- No models available -->
            <template v-else-if="providerSelectionAttempted && displayedProviderModels.length === 0 && !displayedModelsLoading">
              <Alert type="warning">
                <template #title>
              ...
              <FieldInput
                :model-value="activeSpeechModel || ''"
                :label="t('settings.pages.modules.speech.sections.section.model.label')"
                :description="t('settings.pages.modules.speech.sections.section.model.empty-description')"
                :placeholder="t('settings.pages.modules.consciousness.sections.section.provider-model-selection.manual_model_placeholder')"
                @update:model-value="updateCustomModelName"
              />
            </template>
      ...
      <div v-if="hasSelectedSpeechProvider">
        <div flex="~ col gap-4">
          <div>
            ...
            <div class="flex flex-col items-start gap-1 text-neutral-400 md:flex-row md:items-center md:justify-between dark:text-neutral-500">
              ...
              <span v-if="currentSpeechVoiceId" class="text-sm text-neutral-400 font-medium dark:text-neutral-400">
                {{ t('settings.pages.modules.speech.sections.section.voice-configuration.current', { voice: currentSpeechVoiceId }) }}
              </span>
            </div>
          </div>
          ...
          <ErrorContainer
            ...
            class="mb-2"
            :title="t('settings.pages.modules.speech.sections.section.voice-configuration.load-error')"
          ...
          >
            <FieldInput
              ...
              :label="t('settings.pages.modules.speech.sections.section.voice-configuration.manual-voice.label')"
              :description="t('settings.pages.modules.speech.sections.section.voice-configuration.manual-voice.description')"
              :placeholder="t('settings.pages.modules.speech.sections.section.voice-configuration.manual-voice.placeholder')"
            ...
            <div v-if="activeSpeechProvider === 'elevenlabs'">
              <label class="mb-1 block text-sm font-medium">
                {{ t('settings.pages.modules.speech.sections.section.model.label') }}
              </label>
              <select
                v-model="displayedSpeechModel"
                :class="[
    ...
    <section :class="['w-full rounded-2xl p-5', 'glass-tier-1']">
