...
interface OpenAICompatibleValidationOptions<TConfig extends { apiKey?: string, baseUrl?: string }> {
  ...
  /** Resolves effective credentials and endpoints before each network validation. */
  resolveConfig?: (config: TConfig) => TConfig
}
...
export function createOpenAICompatibleValidators<TConfig extends { apiKey?: string, baseUrl?: string }>(
  ...
  async function runChatCheck(
    ...
    const resolvedConfig = options?.resolveConfig?.(config) ?? config
    const model = await pickValidationModel(resolvedConfig, provider, providerExtra)
        ...
        try {
          ...
          if (response.status >= 500) {
            ...
            const reason = options?.connectivityFailureReason
              ? options.connectivityFailureReason({ config: resolvedConfig, error: new Error(errorMessage), errorMessage })
          ...
          const reason = options?.connectivityFailureReason
            ? options.connectivityFailureReason({ config: resolvedConfig, error: e, errorMessage })
            : `Connectivity check failed: ${errorMessage}`
          errors.push({ error: new Error(reason) })
        }
        finally {
  ...
  if (checks.includes(ProviderValidationCheck.ModelList)) {
    validatorConfig.validateProvider?.push(({ t }) => ({
      ...
      validator: async (config, provider, providerExtra) => {
        ...
        const resolvedConfig = options?.resolveConfig?.(config) ?? config
        try {
          const models = await resolveModels(resolvedConfig, provider, providerExtra)
          ...
          const reason = options?.modelListFailureReason
            ? options.modelListFailureReason({ config: resolvedConfig, error: e, errorMessage })
