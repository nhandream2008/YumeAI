...
describe('onboarding store', () => {
  ...
  it('shows the welcome dialog for a guest with an essential provider configured', () => {
  ...
  // ROOT CAUSE:
  //
  // Completing provider setup wrote a permanent completion flag, while the
  // welcome screen's “Don't show this again” checkbox never controlled that
  // flag. The Neko-style welcome dialog could therefore never return for a
  // signed-out guest who left the checkbox clear.
  it('shows again after completion until the guest explicitly suppresses it', () => {
    const store = useOnboardingStore()
