...
describe('migrated provider definitions', () => {
  ...
  it('keeps ElevenLabs configuration and model discovery in the definition', async () => {
    ...
    expect(models?.some(model => model.id === 'eleven_v3')).toBe(true)
    expect(models?.some(model => model.id === 'eleven_flash_v2_5')).toBe(true)
    expect(models?.some(model => model.id === 'eleven_english_sts_v2')).toBe(false)
  })
})
