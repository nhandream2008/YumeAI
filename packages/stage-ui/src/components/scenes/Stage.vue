...
import { defaultLive2DMotionControlDynamics, Live2DScene, useExpressionStore, useLive2DMotionControl, useLive2dParams, useSettingsLive2d } from '@proj-airi/stage-ui-live2d'
...
import { Emotion, EMOTION_EmotionMotionName_value, EMOTION_VRMExpressionName_value, EmotionThinkMotionName, inferEmotionFromText } from '../../constants/emotions'
...
const live2dExpressionStore = useExpressionStore()
...
chatHookCleanups.push(streamingControl.onSignal(async (signal) => {
  if (signal.type === 'act') {
    ...
    if (act.expression && stageModelRenderer.value === 'live2d') {
      const group = live2dExpressionStore.settingsSnapshot.groups.find(item => item.name === act.expression)
      if (group && !group.active && live2dExpressionStore.isExposedToLlm(group.name)) {
        live2dExpressionStore.toggle(group.name, 3)
      }
    }
    return
  }
...
async function playFunction(item: Parameters<Parameters<typeof createPlaybackManager<AudioBuffer>>[0]['play']>[0], signal: AbortSignal): Promise<void> {
  ...
  let lipSyncReady: Promise<void> | undefined
  if (stageModelRenderer.value === 'live2d' && !lipSyncStarted.value) {
    ...
    // the wLipSync graph here when the first audio intent starts.
    setupAnalyser()
    // Do not delay the first audible sample while the worklet loads. Attach the
    // source to the lip-sync node when setup completes.
    lipSyncReady = setupLipSync()
  }
  ...
  if (lipSyncReady) {
    void lipSyncReady.then(() => {
      if (signal.aborted || currentAudioSource.value !== source || !lipSyncNode.value)
        return
      try {
        source.connect(lipSyncNode.value)
      }
      catch {
        // The source can finish while the worklet loads.
      }
    })
  }
...
async function setupLipSync() {
  ...
  // The worklet can take a noticeable amount of time to load. Reuse one
  // in-flight setup so chat startup and the first audio frame do not create
  // duplicate worklet nodes.
  ...
  lipSyncSetupPromise = (async () => {
    try {
      ...
      // The renderer can change while the worklet loads. Do not attach a
      // completed Live2D node to a different renderer.
      if (stageModelRenderer.value !== 'live2d')
        return
...
let assistantEmotionText = ''
let currentTurnId: string | undefined
let fallbackEmotionQueued = false
let modelActTokenSeen = false

async function queueFallbackEmotion(force: boolean) {
  if (fallbackEmotionQueued || modelActTokenSeen)
    return

  const inferredEmotion = inferEmotionFromText(assistantEmotionText)
  if (!inferredEmotion && !force && assistantEmotionText.trim().length < 48)
    return

  fallbackEmotionQueued = true
  const special = `<|ACT ${JSON.stringify({
    emotion: {
      name: inferredEmotion ?? Emotion.Neutral,
      intensity: inferredEmotion ? 0.78 : 0.62,
    },
  })}|>`

  // Keep fallback animation synchronized with speech when a TTS session exists.
  // Without speech output, dispatch it immediately so the avatar still reacts.
  if (!speechMuted.value && currentSession) {
    currentSession.appendSpecial(special)
    return
  }

  await playSpecialToken(special, { turnId: currentTurnId })
}
...
chatHookCleanups.push(onBeforeMessageComposed(async (_message, context) => {
  ...
  assistantEmotionText = ''
  currentTurnId = context.turnId
  fallbackEmotionQueued = false
  modelActTokenSeen = false
  ...
  setupAnalyser()
  // Start lip-sync preparation in parallel with the model request. The first
  // audio playback still awaits this promise before connecting its source.
  void setupLipSync()
...
chatHookCleanups.push(onTokenLiteral(async (literal) => {
  assistantEmotionText += literal
  await queueFallbackEmotion(false)
  currentSession?.appendText(literal)
}))

chatHookCleanups.push(onTokenSpecial(async (special, context) => {
  if (special.trim().startsWith('<|ACT '))
    modelActTokenSeen = true
...
chatHookCleanups.push(onAssistantResponseEnd(async (_message) => {
  await queueFallbackEmotion(true)
  currentSession?.end()
  ...
  <div relative h-full w-full>
    ...
    <div relative h-full w-full>
      <Live2DScene
        ...
        :model-file="stageModelSelectedDisplayModel?.type === 'file' ? stageModelSelectedDisplayModel.file : undefined"
