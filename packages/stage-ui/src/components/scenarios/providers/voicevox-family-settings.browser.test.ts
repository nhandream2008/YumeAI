...
describe('voicevox family settings', () => {
  ...
  // ROOT CAUSE:
  //
  // Provider pages persisted fields in the background but had no explicit
  // action to make that provider the Speech source. A configured engine could
  // therefore still leave the Modules page on "None".
  //
  // The shared settings page now saves the final values, validates the
  // provider, then commits its provider/model pair to the speech store and
  // active card.
  it('saves a configured engine as the active Speech provider', async () => {
    stubReachableEngine()
    const pinia = createPinia()
    ...
    await expect.poll(() => useProviderConfigStore(pinia).providers.voicevox?.status).toBe('configured')
    await screen.getByRole('button', { name: /save-and-use/i }).click()
    await expect.poll(() => speech.activeSpeechProvider).toBe('voicevox')
    await expect.poll(() => speech.activeSpeechModel).toBe('default')
  })
