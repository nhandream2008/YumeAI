...
import { Button, Callout, FieldCheckbox, FieldCombobox, FieldInput, ScrollableArea } from '@proj-airi/ui'
...
const baseUrl = ref('')
const accountId = ref('')
const ollamaConnectionMode = ref<'local' | 'web'>('local')
...
function initializeForm() {
  ...
  apiKey.value = ''
  accountId.value = ''
  ollamaConnectionMode.value = 'local'
...
watch([apiKey, baseUrl, accountId, customFieldValues, ollamaConnectionMode], () => {
...
const needsApiKey = computed(() => {
  ...
  if (hasOnboardingFields.value)
    return false
  if (props.selectedProvider.id === 'ollama')
    return ollamaConnectionMode.value === 'web'

  return props.selectedProvider.id !== 'player2'
})
...
const needsBaseUrl = computed(() => {
  ...
  if (hasOnboardingFields.value)
    return false
  if (props.selectedProvider.id === 'ollama')
    return ollamaConnectionMode.value === 'local'
...
async function validateConfiguration() {
  ...
  try {
    ...
    if (hasOnboardingFields.value) {
      ...
      if (props.selectedProvider.id === 'ollama')
        config.connectionMode = ollamaConnectionMode.value
...
function resolvedCustomFields(): Record<string, string> | undefined {
  if (hasOnboardingFields.value)
    return { ...customFieldValues.value }

  if (props.selectedProvider?.id === 'ollama')
    return { connectionMode: ollamaConnectionMode.value }
    ...
    <ScrollableArea v-if="props.selectedProvider" :class="['min-h-0 flex-1']">
      <div :class="['space-y-4']">
        ...
        <div class="space-y-4">
          ...
          <template v-else>
            <FieldCombobox
              v-if="props.selectedProvider.id === 'ollama'"
              v-model="ollamaConnectionMode"
            ...
            <div v-if="needsApiKey">
              <FieldInput
                v-model="apiKey"
                :placeholder="props.selectedProvider.id === 'ollama'
                  ? t('settings.pages.providers.provider.ollama.fields.api-key.placeholder')
                  : getApiKeyPlaceholder(props.selectedProvider.id)"
                type="password"
                :label="props.selectedProvider.id === 'ollama'
                  ? t('settings.pages.providers.provider.ollama.fields.api-key.label')
                  : 'API Key'"
                :description="props.selectedProvider.id === 'ollama'
                  ? t('settings.pages.providers.provider.ollama.fields.api-key.description')
