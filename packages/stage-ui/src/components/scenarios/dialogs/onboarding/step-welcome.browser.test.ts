...
describe('steam onboarding provider restrictions', () => {
  ...
  it('keeps login without custom provider setup in Steam builds', async () => {
    ...
    expect(document.body.textContent).not.toContain('Try YumeAI')
  })
  ...
  it('keeps custom provider setup in direct builds', async () => {
    ...
    expect(document.body.textContent).toContain('Try YumeAI')

    const logo = document.querySelector('img')
    expect(logo?.getAttribute('width')).toBe('112')
    expect(logo?.getAttribute('height')).toBe('112')
  })
})
