...
import StageHomePage from './pages/index.vue'
...
const keepsStageMounted = computed(() => route.path === '/' || isSettingsRoute.value)
...
onMounted(async () => {
  ...
  // Load persisted display models before a card applies its selected model.
  // Import helpers only run when adding a model, so they stay off the critical
  // startup path that restores the model already selected for the stage.
  await displayModelsStore.loadDisplayModelsFromIndexedDB()
  await cardStore.initialize()
  await settingsStore.initializeStageModel()

  void displayModelsStore.initialize().catch((error) => {
    console.error('Failed to initialize display model import helpers:', error)
  })
...
<template>
  <!--
    Keep one StageHomePage instance alive while Settings changes route. This
    preserves the current chat draft, conversation, and display-model state.
  -->
  <StageHomePage v-show="keepsStageMounted" :overlay-paused="isSettingsRoute" />
  ...
  >
    <RouterView v-if="route.path !== '/'" v-slot="{ Component }">
      <ErrorBoundary
